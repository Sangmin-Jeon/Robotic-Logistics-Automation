...
from b4_fulfillment_interfaces.msg import Button, RobotStatus, ConvGoButton, ConvStopButton
...
class RobotControlUI(QWidget):
    ...
    def initUI(self):
        ...
        self.data_collect_btn.clicked.connect(self.collect_data)
        ...
        self.manual_control_btn.clicked.connect(lambda: self.gui_node.publish_convgobutton_press(True))
        ...
        self.manual_stop_btn.clicked.connect(lambda: self.gui_node.publish_convstopbutton_press(True))
    ...
    def collect_data(self):
        # 학습 데이터 수집 버튼 클릭 시 서비스 호출
        self.gui_node.call_data_collect_service(True)
...
class GUINode(Node):
    def __init__(self):
        ...
        self.convgo_publisher_ = self.create_publisher(ConvGoButton, 'convgo_topic', 10)
        self.convstop_publisher_ = self.create_publisher(ConvStopButton, 'convstop_topic', 10)
        ...
        self.data_collect_client = self.create_client(DataCollect, 'data_collect_service',
                                                      callback_group=self.callback_group)
        self.total = 0
    ...
    def publish_convgobutton_press(self, convmsg):
        msg = ConvGoButton()
        msg.convgobutton = convmsg
        self.convgo_publisher_.publish(msg)  # conv_publisher_ 사용
        self.get_logger().info(f'Published convgobutton press: {msg.convgobutton}')

    def publish_convstopbutton_press(self, convmsg):
        msg = ConvStopButton()
        msg.convstopbutton = convmsg
        self.convstop_publisher_.publish(msg)  # conv_publisher_ 사용
        self.get_logger().info(f'Published convstopbutton press: {msg.convstopbutton}')

    def call_data_collect_service(self, collect):
        req = DataCollect.Request()
        req.start = collect
        future = self.data_collect_client.call_async(req)
        self.get_logger().info(f'Data Collect Start: {req.start}')
        future.add_done_callback(self.data_collect_response_callback)

    def data_collect_response_callback(self, future):
        try:
            response = future.result()
            self.get_logger().info(f'Data collect service response: {response.success}')
